...
static int64_t want_int(const VL_Value *v, int *ok) {
  int64_t x = 0;
...
// ───────────────────────── Helpers ─────────────────────────
static size_t strlcpy_c(char *dst, const char *src, size_t n) {
...
static VL_Value make_cstr(struct VL_Context *ctx, const char *s) {
  return vl_make_strn(ctx, s ? s : "", (uint32_t)(s ? strlen(s) : 0));
}

// ───────────────────────── env ─────────────────────────
static VL_Status os_getenv_fn(struct VL_Context *ctx, const VL_Value *a,
                              uint8_t c, VL_Value *ret, void *ud) {
  (void)ud;
  if (c < 1 || !need_str(&a[0])) return VL_ERR_TYPE;
  const char *v = getenv(a[0].as.s->data);
  if (!v) RET_NIL();
  VL_Value s = make_cstr(ctx, v);
  if (s.type != VT_STR) return VL_ERR_OOM;
  if (ret) *ret = s;
  return VL_OK;
}

static VL_Status os_setenv_fn(struct VL_Context *ctx, const VL_Value *a,
                              uint8_t c, VL_Value *ret, void *ud) {
  (void)ctx;
  (void)ud;
  ...
  char *kv = (char *)malloc(L + 1);
  if (!kv) return VL_ERR_OOM;
  strcpy(kv, a[0].as.s->data);
  strcat(kv, "=");
  strcat(kv, a[1].as.s->data);
  int rc = _putenv(kv);
  free(kv);
  RET_BOOL(rc == 0);
#else
  int rc;
  if (!ow) {
    const char *cur = getenv(a[0].as.s->data);
    if (cur) {
      RET_BOOL(1);
    }
  }
...
static VL_Status os_unsetenv_fn(struct VL_Context *ctx, const VL_Value *a,
                                uint8_t c, VL_Value *ret, void *ud) {
  (void)ctx;
  (void)ud;
  if (c < 1 || !need_str(&a[0])) return VL_ERR_TYPE;
#if defined(_WIN32)
  size_t L = strlen(a[0].as.s->data) + 1;
  char *kv = (char *)malloc(L + 1);
  if (!kv) return VL_ERR_OOM;
  strcpy(kv, a[0].as.s->data);
  strcat(kv, "=");
  ...
  RET_BOOL(rc == 0);
#else
  int rc = unsetenv(a[0].as.s->data);
  RET_BOOL(rc == 0);
#endif
}

static VL_Status os_environ_fn(struct VL_Context *ctx, const VL_Value *a,
                               uint8_t c, VL_Value *ret, void *ud) {
  (void)ud;
  (void)a;
  (void)c;
  ...
  VL_Value s = vl_make_strn(ctx, buf, (uint32_t)n);
  free(buf);
  FreeEnvironmentStringsA(env);
  if (s.type != VT_STR) return VL_ERR_OOM;
  if (ret) *ret = s;
...
// ───────────────────────── cwd / paths ─────────────────────────
static VL_Status os_cwd_fn(struct VL_Context *ctx, const VL_Value *a, uint8_t c,
                           VL_Value *ret, void *ud) {
  (void)a;
  (void)c;
  (void)ud;
  char tmp[4096];
  if (!getcwd(tmp, sizeof(tmp))) return VL_ERR_IO;
  RET_STR(tmp, strlen(tmp));
}
static VL_Status os_chdir_fn(struct VL_Context *ctx, const VL_Value *a,
                             uint8_t c, VL_Value *ret, void *ud) {
  (void)ctx;
  (void)ud;
  if (c < 1 || !need_str(&a[0])) return VL_ERR_TYPE;
  int rc = chdir(a[0].as.s->data);
  RET_BOOL(rc == 0);
}
static VL_Status os_mkdir_p_fn(struct VL_Context *ctx, const VL_Value *a,
                               uint8_t c, VL_Value *ret, void *ud) {
  (void)ud;
  if (c < 1 || !need_str(&a[0])) return VL_ERR_TYPE;
  RET_BOOL(vl_mkdir_p(a[0].as.s->data));
}

// ───────────────────────── system info ─────────────────────────
static VL_Status os_hostname_fn(struct VL_Context *ctx, const VL_Value *a,
                                uint8_t c, VL_Value *ret, void *ud) {
  (void)a;
  (void)c;
  (void)ud;
  ...
  if (!GetComputerNameA(buf, &sz)) return VL_ERR_IO;
  RET_STR(buf, strlen(buf));
#else
  if (gethostname(buf, sizeof(buf)) != 0) return VL_ERR_IO;
  RET_STR(buf, strlen(buf));
#endif
}

static VL_Status os_pid_fn(struct VL_Context *ctx, const VL_Value *a, uint8_t c,
                           VL_Value *ret, void *ud) {
  (void)a;
  (void)c;
  (void)ud;
...
static VL_Status os_cpu_count_fn(struct VL_Context *ctx, const VL_Value *a,
                                 uint8_t c, VL_Value *ret, void *ud) {
  (void)a;
  (void)c;
  (void)ud;
  ...
  SYSTEM_INFO si;
  GetSystemInfo(&si);
  RET_INT((int64_t)(si.dwNumberOfProcessors ? si.dwNumberOfProcessors : 1));
#else
...
static VL_Status os_uname_fn(struct VL_Context *ctx, const VL_Value *a,
                             uint8_t c, VL_Value *ret, void *ud) {
  (void)a;
  (void)c;
  (void)ud;
...
#else
  struct utsname u;
  if (uname(&u) != 0) return VL_ERR_IO;
  snprintf(line, sizeof(line), "%s %s %s %s", u.sysname, u.release, u.version,
           u.machine);
  RET_STR(line, strlen(line));
#endif
}
...
// ───────────────────────── time / sleep ─────────────────────────
static VL_Status os_wall_time_ns_fn(struct VL_Context *ctx, const VL_Value *a,
                                    uint8_t c, VL_Value *ret, void *ud) {
  (void)a;
  (void)c;
  (void)ud;
  ...
  u.LowPart = ft.dwLowDateTime;  // 100ns since 1601
...
static VL_Status os_mono_time_ns_fn(struct VL_Context *ctx, const VL_Value *a,
                                    uint8_t c, VL_Value *ret, void *ud) {
  (void)a;
  (void)c;
  (void)ud;
...
static VL_Status os_sleep_ms_fn(struct VL_Context *ctx, const VL_Value *a,
                                uint8_t c, VL_Value *ret, void *ud) {
  (void)ud;
  if (c < 1) return VL_ERR_INVAL;
  int ok = 1;
  int64_t ms = want_int(&a[0], &ok);
  if (!ok || ms < 0) return VL_ERR_TYPE;
  vl_sleep_ms((uint32_t)ms);
  RET_NIL();
}

// ───────────────────────── system / exec ─────────────────────────
static VL_Status os_system_fn(struct VL_Context *ctx, const VL_Value *a,
                              uint8_t c, VL_Value *ret, void *ud) {
  (void)ctx;
  (void)ud;
  if (c < 1 || !need_str(&a[0])) return VL_ERR_TYPE;
  int rc = system(a[0].as.s->data);
  RET_INT((int64_t)rc);
}
static VL_Status os_exec_fn(struct VL_Context *ctx, const VL_Value *a,
                            uint8_t c, VL_Value *ret, void *ud) {
  (void)ud;
  if (c < 1 || !need_str(&a[0])) return VL_ERR_TYPE;
  char *out = NULL;
  size_t n = 0;
  int st = 0;
  if (!vl_exec_capture(a[0].as.s->data, &out, &n, &st)) return VL_ERR_IO;
  VL_Value s = vl_make_strn(ctx, out, (uint32_t)n);
  free(out);
  if (s.type != VT_STR) return VL_ERR_OOM;
  if (ret) *ret = s;
  return VL_OK;
...
// ───────────────────────── temp / isatty ─────────────────────────
static VL_Status os_tempdir_fn(struct VL_Context *ctx, const VL_Value *a,
                               uint8_t c, VL_Value *ret, void *ud) {
  (void)a;
  (void)c;
  (void)ud;
  ...
  DWORD m = GetTempPathA((DWORD)sizeof(buf), buf);
  if (m == 0 || m >= sizeof(buf)) return VL_ERR_IO;
  td = buf;
  RET_STR(td, strlen(td));
#else
  td = getenv("TMPDIR");
  if (!td || !*td) td = "/tmp";
  RET_STR(td, strlen(td));
...
static VL_Status os_isatty_fn(struct VL_Context *ctx, const VL_Value *a,
                              uint8_t c, VL_Value *ret, void *ud) {
  (void)ud;
  int fd = 1, ok = 1;
  if (c >= 1) {
    int64_t x = want_int(&a[0], &ok);
    if (!ok) return VL_ERR_TYPE;
    fd = (int)x;
  }
  RET_BOOL(isatty(fd));
...
// ───────────────────────── chmod / umask (POSIX) ─────────────────────────
static VL_Status os_chmod_fn(struct VL_Context *ctx, const VL_Value *a,
                             uint8_t c, VL_Value *ret, void *ud) {
  (void)ud;
  if (c < 2 || !need_str(&a[0])) return VL_ERR_TYPE;
  int ok = 1;
  int64_t mode = want_int(&a[1], &ok);
  if (!ok) return VL_ERR_TYPE;
#if defined(_WIN32)
  (void)mode;
  RET_BOOL(0);
#else
  int rc = chmod(a[0].as.s->data, (mode_t)mode);
  RET_BOOL(rc == 0);
#endif
}

static VL_Status os_umask_fn(struct VL_Context *ctx, const VL_Value *a,
                             uint8_t c, VL_Value *ret, void *ud) {
  (void)ctx;
  (void)ud;
#if defined(_WIN32)
  ...
  int ok = 1;
  int64_t m = want_int(&a[0], &ok);
  if (!ok) return VL_ERR_TYPE;
  mode_t old = umask((mode_t)m);
  RET_INT((int64_t)old);
#endif
}

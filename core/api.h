...
/* Fournit des helpers inline pour composer Err */
...
/* Temps / sommeil */
/* -------------------------------------------------------------------------- */
API_EXPORT u64 time_ns_monotonic(void); /* horloge monotone en ns */
API_EXPORT u64 time_ms_wall(void);      /* temps réel en ms depuis epoch */
...
API_EXPORT u64 rand_range_u64(u64 lo, u64 hi); /* inclusif */
...
#define vec_reserve(v, need)                                           \
  do {                                                                 \
    ...
    if (!(v)->data) {                                                  \
      fprintf(stderr, "vec_reserve OOM\n");                            \
      abort();                                                         \
    }                                                                  \
    (v)->cap = _ncap;                                                  \
  } while (0)

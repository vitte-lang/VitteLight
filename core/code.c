...
Err code_emit_demo_json(int argc, char** argv, const char* out_path,
  ...
  jw_obj_begin(&jw);
  jw_key(&jw, "app");
  jw_str(&jw, CODE_APP_NAME);
  jw_key(&jw, "version");
  jw_str(&jw, CODE_APP_VERSION);
  jw_key(&jw, "time_ms");
  jw_i64(&jw, (i64)time_ms_wall());
  jw_key(&jw, "rand");
  jw_i64(&jw, (i64)(rand_u64() & 0xffffffffu));
  jw_key(&jw, "args");
  jw_arr_begin(&jw);
...
Err code_freq_pairs(const char* path, VEC(CodeKV) * out_pairs) {
  ...
  for (usize s = 0; s < map.cap; s++) {
    ...
    if (slot->used) {
      CodeKV kv;
      kv.word = xstrdup(slot->key); /* copie pour indépendance */
      kv.count = slot->val;
...
bool code_cmd_parse(const char* s, CodeCmd* out_cmd) {
  ...
  struct {
    const char* n;
    CodeCmd c;
  } map[] = {{"help", CMD_HELP}, {"info", CMD_INFO}, {"rand", CMD_RAND},
...
int code_main(int argc, char** argv) {
  ...
  switch (cmd) {
    ...
    case CMD_RAND: {
      ...
      if (e.code) {
        logf(LOG_ERROR, "%s", e.msg);
        sb_free(&sb);
        return code_status_from_err(&e);
      }
    ...
    case CMD_HASH: {
      if (argc < 3) {
        logf(LOG_ERROR, "hash: besoin d’un chemin");
        return CODE_EINVAL;
      }
      u64 h = 0;
      Err e = code_hash_file(argv[2], &h);
      if (e.code) {
        logf(LOG_ERROR, "%s", e.msg);
        return code_status_from_err(&e);
      }
      printf("%016llx  %s\n", (unsigned long long)h, argv[2]);
      return 0;
    }

    case CMD_CAT: {
      if (argc < 3) {
        logf(LOG_ERROR, "cat: besoin d’un chemin");
        return CODE_EINVAL;
      }
      StrBuf sb;
      sb_init(&sb);
    ...
    case CMD_UTF8: {
      if (argc < 3) {
        logf(LOG_ERROR, "utf8: besoin d’un texte");
        return CODE_EINVAL;
      }
      VEC(u32) cps;
      Err e = code_utf8_list_cps(argv[2], &cps);
      if (e.code) {
        logf(LOG_ERROR, "%s", e.msg);
        return code_status_from_err(&e);
      }
      for (usize i = 0; i < cps.len; i++)
        printf("U+%04X\n", (unsigned)cps.data[i]);
      vec_free(&cps);
      return 0;
    }
    ...
    case CMD_FREQ: {
      if (argc < 3) {
        logf(LOG_ERROR, "freq: besoin d’un fichier");
        return CODE_EINVAL;
      }
      int topk = (argc >= 4) ? atoi(argv[3]) : 20;
      VEC(CodeKV) xs;
      Err e = code_freq_pairs(argv[2], &xs);
      if (e.code) {
        logf(LOG_ERROR, "%s", e.msg);
        return code_status_from_err(&e);
      }
      ...
      for (int i = 0; i < limit; i++) {
        ...
        free((void*)xs.data[i].word); /* libère les duplicatas */
      }
      vec_free(&xs);
      return 0;
    }
    ...
    case CMD_BENCH: {
      size_t bytes =
          (argc >= 3) ? (size_t)strtoull(argv[2], NULL, 10) : ((size_t)1 << 20);
      int iters = (argc >= 4) ? atoi(argv[3]) : 200;
      CodeBench r;
      Err e = code_bench_hash64(bytes, iters, &r);
      if (e.code) {
        logf(LOG_ERROR, "%s", e.msg);
        return code_status_from_err(&e);
      }
      printf("hash64: %.3fs, %.2f GiB, %.2f GiB/s, acc=%016llx\n", r.seconds,
             r.gib, r.gib_per_s, (unsigned long long)r.accumulator);
      return 0;
    }

    case CMD_ANSI: {
      if (argc < 3) {
        logf(LOG_ERROR, "ansi: besoin d’un texte");
        return CODE_EINVAL;
      }
      StrBuf sb;
      sb_init(&sb);
